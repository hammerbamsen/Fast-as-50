# Søndagens opgave: uge-gennemgang + næste uges fokus

Det er søndag {date}. Ugen er slut. Gør ugen op og læg fokus for næste uge. Svar via `coach_output`.
...
7. `warnings`: næste uges brud på 72 t-reglen (`week.hardSpacing`), kvote over `nextWeek.quota`, hårde pas for tæt på `nextRace`. Kælderpas vælges KUN fra `catalog` (id) — foreslå aldrig et pas der ikke står der, og ændr aldrig et pas' watt.
